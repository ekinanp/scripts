...
run_beaker() {
  ...
  bundle exec beaker --hosts ${hosts_cfg} --type pe --keyfile ${BEAKER_KEYFILE} --tests ${BEAKER_TESTSUITE} --preserve-hosts onfail --helper ${BEAKER_HELPER} --pre-cleanup ${BEAKER_PRECLEANUP} --pre-suite ${BEAKER_PRESUITE} --log-prefix ${results_dir} --log-level verbose
